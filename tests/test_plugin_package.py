...
def test_plugin_mcp_configs_use_the_deployed_http_endpoint() -> None:
    claude_server = _read_json(".mcp.json")["mcpServers"]["daily-notes"]
    codex_server = _read_json(".codex-mcp.json")["daily-notes"]
...
def test_marketplaces_expose_the_plugin_from_the_repository_root() -> None:
    ...
    assert claude_plugin["name"] == codex_plugin["name"] == "daily-notes-mcp"
