...
export function createRevision(content: string): string {
  return `sha256:${sha256(normalizeContent(content))}`;
}
...
export function randomId(prefix: string): string {
  ...
  if (secureRandom === undefined) {
    throw new Error("Secure random values are unavailable.");
  }
  let random = secureRandom.randomUUID?.();
  if (random === undefined) {
    const bytes = secureRandom.getRandomValues(new Uint8Array(16));
    bytes[6] = ((bytes[6] ?? 0) & 0x0f) | 0x40;
    bytes[8] = ((bytes[8] ?? 0) & 0x3f) | 0x80;
    const hex = [...bytes].map((byte) => byte.toString(16).padStart(2, "0"));
    random = `${hex.slice(0, 4).join("")}-${hex.slice(4, 6).join("")}-${hex.slice(6, 8).join("")}-${hex.slice(8, 10).join("")}-${hex.slice(10).join("")}`;
  }
